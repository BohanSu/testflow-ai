...
export class CursorProvider implements AIProvider {
  ...
  capabilities: ProviderCapabilities = {
    ...
    supportedFrameworks: ['playwright', 'jest', 'vitest', 'mocha'],
    maxConcurrency: 3,
    canRunTests: true,
    canDebug: true,
    canAnalyze: true,
  ...
  private async runTest(testLocation: string, retries: number): Promise<ProviderToolResponse> {
    const env = this.apiKey ? { ...process.env, CURSOR_API_KEY: this.apiKey } : process.env;

    try {
      const { stdout, stderr } = await execAsync(
        `cursor test run ${testLocation} ${retries > 1 ? `--retries=${retries}` : ''}`,
  ...
  private async getFailureReport(testLocation: string): Promise<ProviderToolResponse> {
    ...
    try {
      const { stdout, stderr } = await execAsync(
        `cursor test report ${testLocation} --format=json`,
  ...
  private async triageTests(testPath: string): Promise<ProviderToolResponse> {
    ...
    try {
      const { stdout } = await execAsync(
        `cursor test triage ${testPath || '.'} --output=json`,
  ...
  private async getEvidenceBundle(testLocation: string): Promise<ProviderToolResponse> {
    ...
    try {
      const { stdout } = await execAsync(
        `cursor test evidence ${testLocation} --format=json`,
